...
- [Permission Modes](permission-modes.md)：默认权限、Full Access、工作空间和高风险确认的决策规则。
- [Security Checklist](../templates/security-checklist.md)：执行前最小安全检查。
...
- 对项目级上下文文档采用只读优先：它们是实践约定，不是 WorkBuddy 官方目录规范，也不等同于官方 Memory。
...
已分享文件应定期清理。对外分享前要检查内容、接收范围和保密标记；不再需要公开访问时，及时取消分享。取消分享只应理解为停止原分享链接继续访问，不应扩写成删除接收人已下载文件、下游副本、日志、导出件或企业系统留痕。
